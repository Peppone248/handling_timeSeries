...
def liner_interp(t_in, y_in, t_out):
    t_in = np.asarray(t_in, dtype=float)
    y_in = np.asarray(y_in, dtype=float)
    t_out = np.asarray(t_out, dtype=float)
    ...
    for i in range(len(t_out)):
        ...
        if current_time < t_in[0]:
            pass
        else:
            i_right = idx[i]
            i_left = idx[i] - 1

            # caso esatto sull'ultimo campione
            if i_right == len(t_in):
                out_val[i] = y_in[-1]
                continue

            t_left = t_in[i_left]
            t_right = t_in[i_right]

            y_left = y_in[i_left]
            y_right = y_in[i_right]

            # coefficiente di interpolazione
            alpha = (current_time - t_left) / (t_right - t_left)

            out_val[i] = y_left + alpha * (y_right - y_left)
